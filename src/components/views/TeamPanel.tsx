...
import { UserProfile, Team, TeamMember, JoinRequest } from '../../types';
...
export function TeamPanel({ profile, team, members, onApprove, onReject }: TeamPanelProps) {
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, snap => setRequests(snap.docs.map(d => ({ id: d.id, ...d.data() } as JoinRequest))));
  }, [team, profile.role]);
  ...
  if (profile.role === 'pending') return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto space-y-6">
      <div className="bg-white p-8 rounded-2xl border border-[rgba(0,0,0,0.08)] shadow-sm text-center">
        ...
        <p className="text-gray-500 text-sm mb-4">Your join request has been sent. Watch the bell icon for your notification.</p>
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 mt-1 mb-4">
              <div className="flex items-center gap-1.5 text-xs font-bold text-gray-400 uppercase tracking-wider">
                <ShieldCheck size={12} /> Role: <span className="text-[#1D9E75]">{profile.role}</span>
              </div>
              <div className="flex items-center gap-1.5 text-xs font-bold text-gray-400 uppercase tracking-wider">
              ...
              <div className="bg-[#F8F7FF] border border-[#E0DEFF] rounded-2xl p-5">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex-1 bg-white border-2 border-dashed border-[#C4BFFE] rounded-xl px-6 py-4 text-center">
                    <span className="text-3xl font-mono font-black tracking-[0.3em] text-[#534AB7]">{team.joinCode}</span>
                  </div>
                  <button onClick={copyCode}
                    className={`flex flex-col items-center gap-1.5 px-5 py-3 rounded-xl text-sm font-bold transition-all shrink-0 ${copied ? 'bg-[#1D9E75] text-white' : 'bg-white border border-[rgba(0,0,0,0.1)] text-gray-700 hover:bg-gray-50'}`}>
                    {copied ? <Check size={18} /> : <Key size={18} />}
                    <span className="text-[11px]">{copied ? 'Copied!' : 'Copy'}</span>
                  </button>
                </div>
                <p className="text-[11px] text-gray-400 mt-3 text-center">This code stays here until you navigate away — no timer.</p>
              </div>
            )}
          </div>
        </div>
          ...
          {requests.map(req => (
            <div key={req.id} className="bg-white p-4 rounded-xl border border-[rgba(0,0,0,0.08)] flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={() => onApprove(req.id, req.userId)} className="bg-[#1D9E75] text-white px-4 py-1.5 rounded-lg text-xs font-bold hover:bg-[#168361] flex items-center gap-1">
                  <Check size={12} /> Approve
                </button>
                <button onClick={() => onReject(req.id, req.userId)} className="bg-gray-100 text-gray-600 px-4 py-1.5 rounded-lg text-xs font-bold hover:bg-gray-200 flex items-center gap-1">
                  <X size={12} /> Decline
                </button>
              </div>
            </div>
