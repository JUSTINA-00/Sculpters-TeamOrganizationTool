...
function LinkPopup({ onInsert, onClose }: { onInsert: (url: string) => void; onClose: () => void }) {
  ...
  return (
    <div className="absolute z-50 top-full left-0 mt-1 bg-white border border-[rgba(83,74,183,0.25)] rounded-xl shadow-lg p-3 flex gap-2 items-center">
      ...
      />
      <button onMouseDown={e => { e.preventDefault(); onInsert(url); }}
        className="bg-[#534AB7] text-white text-sm px-3 py-1.5 rounded-lg font-semibold hover:bg-[#453d9c]">Insert</button>
  ...
  return (
    <div className="absolute z-50 top-full left-0 mt-1 bg-white border border-[rgba(83,74,183,0.25)] rounded-xl shadow-lg p-4 w-72 flex flex-col gap-3">
      ...
      <div className="flex gap-2">
        <button onMouseDown={e => { e.preventDefault(); if (url.trim()) onInsert(url.trim()); }}
          className="flex-1 bg-[#534AB7] text-white text-sm py-1.5 rounded-lg font-semibold hover:bg-[#453d9c]">Insert</button>
        <button onMouseDown={e => { e.preventDefault(); onClose(); }}
          className="flex-1 bg-gray-100 text-gray-600 text-sm py-1.5 rounded-lg hover:bg-gray-200">Cancel</button>
      </div>
    </div>
  );
}
...
interface HandleDef {
  ...
  cursor: string;
  dx: -1 | 0 | 1;   // does this handle affect width, and in which direction?
  dy: -1 | 0 | 1;   // does this handle affect height, and in which direction?
}

const HANDLES: HandleDef[] = [
  { id: 'nw', style: { top: 0,   left: 0,   transform: 'translate(-50%,-50%)' }, cursor: 'nw-resize', dx: -1, dy: -1 },
  { id: 'n',  style: { top: 0,   left: '50%', transform: 'translate(-50%,-50%)' }, cursor: 'n-resize',  dx:  0, dy: -1 },
  { id: 'ne', style: { top: 0,   right: 0,  transform: 'translate(50%,-50%)'  }, cursor: 'ne-resize', dx:  1, dy: -1 },
  { id: 'e',  style: { top: '50%', right: 0, transform: 'translate(50%,-50%)'  }, cursor: 'e-resize',  dx:  1, dy:  0 },
  { id: 'se', style: { bottom: 0, right: 0,  transform: 'translate(50%,50%)'   }, cursor: 'se-resize', dx:  1, dy:  1 },
  { id: 's',  style: { bottom: 0, left: '50%', transform: 'translate(-50%,50%)' }, cursor: 's-resize',  dx:  0, dy:  1 },
  { id: 'sw', style: { bottom: 0, left: 0,  transform: 'translate(-50%,50%)'  }, cursor: 'sw-resize', dx: -1, dy:  1 },
  { id: 'w',  style: { top: '50%', left: 0, transform: 'translate(-50%,-50%)' }, cursor: 'w-resize',  dx: -1, dy:  0 },
];
...
function ResizeOverlay({ img, scrollContainer, onResize, onDeselect }: ResizeOverlayProps) {
  ...
  const updateRect = useCallback(() => {
    ...
    const r = {
      top:    ir.top  - cr.top  + scrollContainer.scrollTop,
      left:   ir.left - cr.left,
      width:  ir.width,
      height: ir.height,
    };
    setRect(r);
    setSizeLabel(`${Math.round(r.width)} × ${Math.round(r.height)}`);
  }, [img, scrollContainer]);
  ...
  }, [updateRect, scrollContainer]);

  const makeHandleMouseDown = useCallback((h: HandleDef) => (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      if (h.dx !== 0) {
        const newW = Math.max(48, startW + h.dx * dx);
        img.style.width  = `${newW}px`;
        img.style.maxWidth = '100%';
      }
      if (h.dy !== 0) {
        const newH = Math.max(48, startH + h.dy * dy);
  ...
  return (
    <>
      {/* Click-away backdrop */}
      <div className="fixed inset-0 z-30" onMouseDown={onDeselect} />
      ...
      >
        {/* Selection border */}
        <div className="absolute inset-0 rounded-xl ring-2 ring-[#534AB7]" />

        {/* 8 handles */}
        {HANDLES.map(h => (
          <div
            key={h.id}
            className="absolute w-3 h-3 bg-white border-2 border-[#534AB7] rounded-full pointer-events-auto"
            style={{ ...h.style, cursor: h.cursor }}
            onMouseDown={makeHandleMouseDown(h)}
          />
        ))}

        {/* Size badge */}
        <div className="absolute -bottom-7 left-1/2 -translate-x-1/2 bg-[#534AB7] text-white text-[10px] font-mono px-2 py-0.5 rounded-full whitespace-nowrap pointer-events-none select-none">
          {sizeLabel}
        </div>
      </div>
    </>
...
function RichEditor({ initialContent, onChange }: { initialContent: string; onChange: (html: string) => void }) {
  ...
  // Attach click handlers to every img in the editor
  useEffect(() => {
    const editor = editorRef.current;
    if (!editor) return;
    const handleImgClick = (e: MouseEvent) => {
      e.stopPropagation();
      setSelectedImg(e.currentTarget as HTMLImageElement);
    };
    const attach = () => {
  ...
  const triggerSave = useCallback(() => {
    ...
    saveTimer.current = setTimeout(() => onChange(editorRef.current?.innerHTML ?? ''), 800);
  }, [onChange]);
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      {/* Scroll container — position:relative anchors the overlay */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto relative">
        <div
          ...
          className="min-h-full px-8 py-6 outline-none text-gray-800 leading-relaxed
        ...
        {selectedImg && scrollRef.current && (
          <ResizeOverlay
            img={selectedImg}
            scrollContainer={scrollRef.current}
            onResize={triggerSave}
  ...
  if (activeDoc) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <div className="flex-1 flex flex-col bg-white border border-[rgba(0,0,0,0.08)] rounded-2xl shadow-sm overflow-hidden">
        <RichEditor initialContent={activeDoc.content} onChange={html => onUpdate(activeDoc.id, html)} />
      </div>
    </motion.div>
  );
