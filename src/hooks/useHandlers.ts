...
export function useHandlers({
  ...
  const handleSignup = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newProfile: UserProfile = {
      ...
      color: pickColor(user.uid), role: 'pending', teamId: null, createdAt: Date.now(),
    };
  ...
  const handleCreateTeam = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const teamData: Team = { id: teamRef.id, name: teamName, joinCode, leaderId: firebaseUser.uid, createdAt: Date.now() };
      await setDoc(teamRef, teamData);
      await updateDoc(doc(db, 'users', firebaseUser.uid), { teamId: teamRef.id, role: 'leader' });
      setActiveTeamId(teamRef.id); setShowNewTeamPanel(false); triggerSync();
  ...
  const handleJoinTeam = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const existingQ = query(collection(db, 'joinRequests'), where('userId', '==', firebaseUser.uid), where('teamId', '==', foundTeam.id), where('status', '==', 'pending'));
      if (!(await getDocs(existingQ)).empty) { alert('You already have a pending request.'); return; }
      await addDoc(collection(db, 'joinRequests'), { userId: firebaseUser.uid, teamId: foundTeam.id, userName: profile.displayName, userEmail: profile.email, status: 'pending', createdAt: Date.now() });
      await updateDoc(doc(db, 'users', firebaseUser.uid), { teamId: foundTeam.id, role: 'pending' });
      setActiveTeamId(foundTeam.id); setShowNewTeamPanel(false); triggerSync();
  ...
  const handleApproveRequest = async (requestId: string, userId: string) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'users', userId), { teamId: activeTeamId, role: 'member' });
      await addDoc(collection(db, 'notifications'), { userId, message: `Your request to join "${team.name}" has been approved! You now have full access.`, read: false, type: 'approval', createdAt: Date.now() });
      triggerSync();
  ...
  const handleRejectRequest = async (requestId: string, userId: string) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'notifications'), { userId, message: `Your request to join "${team.name}" was declined.`, read: false, type: 'rejection', createdAt: Date.now() });
    } catch (err) { handleFirestoreError(err, OperationType.UPDATE, `joinRequests/${requestId}`); }
  };
  ...
  const handleAddTask = async (title: string, tag: TaskTag, assignedTo: string | null) => {
    ...
    const now = Date.now();
    try { await addDoc(collection(db, 'tasks'), { title, tag, completed: false, assignedTo, createdBy: firebaseUser.uid, teamId: activeTeamId, createdAt: now, updatedAt: now }); triggerSync(); }
    catch (err) { handleFirestoreError(err, OperationType.CREATE, 'tasks'); }
  };

  const handleToggleTask = async (task: Task) => {
    try { await updateDoc(doc(db, 'tasks', task.id), { completed: !task.completed, updatedAt: Date.now() }); triggerSync(); }
    catch (err) { handleFirestoreError(err, OperationType.UPDATE, `tasks/${task.id}`); }
  };

  const handleDeleteTask = async (taskId: string) => {
    try { await deleteDoc(doc(db, 'tasks', taskId)); triggerSync(); }
  ...
  const handleAssignTask = async (taskId: string, assignedTo: string | null) => {
    try { await updateDoc(doc(db, 'tasks', taskId), { assignedTo, updatedAt: Date.now() }); triggerSync(); }
  ...
  const handleCreateDoc = async (name: string, emoji: string) => {
    if (!activeTeamId || !firebaseUser) return;
    const now = Date.now();
    try { await addDoc(collection(db, 'docs'), { name, emoji, content: '', teamId: activeTeamId, createdBy: firebaseUser.uid, editors: [firebaseUser.uid], createdAt: now, updatedAt: now }); triggerSync(); }
  ...
  const handleUpdateDoc = async (docId: string, content: string) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'docs', docId), { content, updatedAt: Date.now(), editors }); triggerSync();
    } catch (err) { handleFirestoreError(err, OperationType.UPDATE, `docs/${docId}`); }
  };

  const handleDeleteDoc = async (docId: string) => {
    try { await deleteDoc(doc(db, 'docs', docId)); triggerSync(); }
  ...
  const handleCreateMeeting = async (data: { title: string; description: string; time: string; date: string }) => {
    if (!activeTeamId || !firebaseUser) return;
    try { await addDoc(collection(db, 'meetings'), { ...data, teamId: activeTeamId, createdBy: firebaseUser.uid, attendees: [firebaseUser.uid], rsvps: [firebaseUser.uid], createdAt: Date.now() }); triggerSync(); }
  ...
  const handleRsvpMeeting = async (meeting: Meeting) => {
    ...
    try { await updateDoc(doc(db, 'meetings', meeting.id), { rsvps: updatedRsvps }); triggerSync(); }
    catch (err) { handleFirestoreError(err, OperationType.UPDATE, `meetings/${meeting.id}`); }
  };

  const handleDeleteMeeting = async (meetingId: string) => {
    try { await deleteDoc(doc(db, 'meetings', meetingId)); triggerSync(); }
